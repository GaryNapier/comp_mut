...
if (TESTING){
  ...
  PCM_file <- "metadata/found_PCM.csv"
  outfile <- sprintf("results/%s_PCM_KCM_merged.csv", drug_of_interest)
}else{
  ...
  option_list = list(
    ...
    make_option(c("-k", "--KCM_file"), type="character", default=NULL,
                help="file of known compensatory mutations", metavar="character"),
    make_option(c("-p", "--PCM_file"), type="character", default=NULL,
                help="file of known compensatory mutations", metavar="character"),
...
PCM <- read.csv(PCM_file, header = T)
...
# Put the KCM and PCM together
KCM <- select(subset(KCM, Drug == drug_of_interest), Gene, Mutation)
PCM <- select(subset(PCM, drug == drug_of_interest), gene, mutation)
CM <- rbind_force(KCM, PCM)
names(CM) <- c('gene', 'change')
...
if(drug_of_interest == "isoniazid"){
  ...
  CM <- CM[!(grepl("p.", CM$change)), ]
  
}

if (!TESTING){
  write.csv(CM, file = outfile, quote = F, row.names = F)
}
