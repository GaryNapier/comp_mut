...
```{r CM-kat315-PRM-table, echo = F, warning=F}
...
CM_kat315_PRM_table <- merge(inh_kat315_no_PRM_pivot, inh_PRM_no_katG_KRM_pivot, 
                             by = "CM_bin", sort = F)

names(CM_kat315_PRM_table) <- c("CM", "katG-p.Ser315Thr", "PRM")

# Do chi-sq test
x <- table(inh_kat315_no_PRM$CM_bin, inh_kat315_no_PRM$KRM)
y <- table(inh_PRM_no_katG_KRM$CM_bin, inh_PRM_no_katG_KRM$PRM_bin)
tab <- cbind(x, y)
x_sq <- print_chisq(chisq.test(tab), rnd = rnd)
...
**Table \@ref(tab:PRM-KRM-other-table)** shows the co-occurrence of the INH potential resistance mutation haplotypes and co-occurrence of known resistance mutations and unknown mutations in relevant resistance genes (i.e. co-occurrence of *katG* potential resistance mutations with known/unknown *ahpC*, *inhA*, *fabG1* and *kasA*). There were `r inh_n_PRM_hap_KRM` (`r fmt_pc(inh_n_PRM_hap_KRM/inh_n_PRM_haplotypes)`) *katG* potential resistance mutation haplotypes occurring with a known resistance mutation (`r sum(inh_PRM_KRM$n)` samples), the most common of which was `r inh_most_common_KRM_with_PRM`.
