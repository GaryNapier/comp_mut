...
rare_katg_table <- subset(isoniazid_binary_table, is.na(KRM) & !(is.na(CM)) & is.na(PRM) & !(is.na(katG)) )
...
rare_rpob_table <- subset(rifampicin_binary_table, is.na(KRM) & !(is.na(CM)) & is.na(PRM) & !(is.na(rpoB)) )
...
# total_samps
# # 2
# nrow(subset(inh_bin, KRM_bin == "present"))
# nrow(subset(inh_bin, KRM_bin == "absent"))
# # 3
# nrow(subset(inh_bin, KRM_bin == "present" &
#          CM_bin == "present"))
# nrow(subset(inh_bin, KRM_bin == "present" &
...
```{r results-stats, echo = F, warning=F}
...
# Number of samples having a CM which also have a PRM
inh_n_samps_CM_and_PRM <- nrow(subset(inh_bin, CM_bin == "present" & PRM_bin == "present"))
# Number of samples having a PRM but no CM
inh_n_samps_PRM_no_CM <- nrow(subset(inh_bin, CM_bin == "absent" & PRM_bin == "present"))
# Number of samples having a CM which also have a PRM
rif_n_samps_CM_and_PRM <- nrow(subset(rif_bin, CM_bin == "present" & PRM_bin == "present"))
# Number of samples having a PRM but no CM
rif_n_samps_PRM_no_CM <- nrow(subset(rif_bin, CM_bin == "absent" & PRM_bin == "present"))
...
Number INH samples having a CM which also have a PRM - `r inh_n_samps_CM_and_PRM` \
Number INH samples having a PRM but no CM - `r inh_n_samps_PRM_no_CM` \

Number RIF samples having a CM which also have a PRM - `r rif_n_samps_CM_and_PRM` \
Number RIF samples having a PRM but no CM - `r rif_n_samps_PRM_no_CM`

<!-- HOW MANY SAMPS WITH PRM HAVE PUTATIVE CM? -->
