...
KRM_pc_box <- ggplot()+
  ...
  # geom_dotplot(data = KRM_PRM_CM_pc_tab_plot,
  #              aes(x = resistance_level, y = KRM_pc, fill = status),
...
grid.arrange(KRM_pc_box, CM_pc_box, ncol=2)
...
cap <- "Percentages of co-occurring known resistance mutations and compensatory mutations among known and putative ('unknown') resistance mutations. Mutations in fewer than three samples are oomitted. Co-occurring known resistance mutations are in genes *fabG1*, *inhA*, *kasA*. Classes of resistance level are based on Ser315Thr: High resistance level < 15% co-occurring known resistance mutations, medium resistance level >= 15% < 90%  co-occurring known resistance mutations, high resistance level >= 90% co-occurring known resistance mutations; high fitness cost > 90% co-occurring compensatory mutations, medium fitness cost < 90% > 3% co-occurring compensatory mutations, low fitness cost <= 3% co-occurring compensatory mutations"
...
```{r homoplasy-plot, echo=FALSE, warning = F, out.width = "2000px", fig.cap="Percentage of samples with co-occurring known resistance mutations (x-axis) and percentage of samples with a compensatory mutation (y-axis) in each of the known and putative *katG* resistance mutations. Ser315Thr is highlighted. As a known mutation which incurrs little fitness cost, a low percentage of its samples also have compensatory mutations. This mutation also conferrs a high level of resistance to INH, hence its samples also co-occur with relatively few other (non-*katG*) resistance mutations. Mutations in fewer than three samples are omitted."}
