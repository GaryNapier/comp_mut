...
distances_file <- paste0(results_path, "distances_with_mutations.txt")
...
# Write out 
write.csv(binary_tables$isoniazid, file = paste0(results_path, "isoniazid_binary_table_clean.csv"), quote = F, row.names = F)
...
# Distances file
distances$status <- ifelse(distances$status == "novel", "putative", distances$status)
...
# Merge in sublin
inh_bin <- merge(inh_bin, select(lineage_data, id, sublin), by.x = "wgs_id", 
                 by.y = "id", all.x = T, sort = F)
inh_bin <- dplyr::rename(inh_bin, sublin = sublin.y)
...
table_1 <- rm_dup_group(table_1, "Characteristic")
...
```{r n-PRM-KRM-basic stats, echo = F, warning = F}

# n katG mutations
inh_n_uniq_KRM_katg <- length(unique(unlist(strsplit(unique(inh_bin[!(is.na(inh_bin[, "KRM_katG"])), "KRM_katG"]), "; "))))
...
gene_mutation <- do.call("rbind", lapply(str_split(common_KRM_tab$KRM, "-c.|-p."), function(x){
  data.frame(Gene = x[1], mutation = x[2])
}))
common_KRM_tab <- cbind(common_KRM_tab, gene_mutation)


# GET STATS BEFORE FILTERING BY MIN SAMPLES

common_KRM_tab$pos <- find_pos(common_KRM_tab$KRM)

n_katg_greater_2 <- nrow(subset(common_KRM_tab, Gene == "katG" & n > 2))
n_katg_greater_4 <- nrow(subset(common_KRM_tab, Gene == "katG" & n > 4))
n_rpob_greater_4 <- nrow(subset(common_KRM_tab, Gene == "rpoB" & n > 4))

n_katg_greater_2_ins_del_dup_promoter <- nrow( subset(common_KRM_tab, Gene == "katG" & n > 2 & (grepl("del|dup|ins", KRM) |  pos < 0) ) )
n_katg_greater_2_non_ins_del_dup_non_promoter <- nrow( subset(common_KRM_tab, Gene == "katG" & n > 2 & !(grepl("del|dup|ins", KRM)) & pos > 0  ) )

common_KRM_tab <- dplyr::select(common_KRM_tab, -(pos))

# FILTER BY MIN SAMPLES FOR DISPLAY

common_KRM_tab <- subset(common_KRM_tab, n > 4)
common_KRM_tab <- dplyr::select(common_KRM_tab, -(KRM))
gene_order <- c("katG", "fabG1", "inhA", "ahpC", "rpoB", "rpoC")
common_KRM_tab <- common_KRM_tab %>% arrange(factor(Gene, levels = gene_order))
common_KRM_tab$mutation <- paste0(common_KRM_tab$mutation, " [", common_KRM_tab$n, "]")
common_KRM_tab <- dplyr::select(common_KRM_tab, -(n))
common_KRM_tab <- rm_dup_group(common_KRM_tab, "Drug")
common_KRM_tab <- rm_dup_group(common_KRM_tab, "Gene")
names(common_KRM_tab) <- c("Drug", "Gene", "Mutation [freq]")

```

Our data contained `r inh_n_uniq_KRM_katg` distinct known *katG* resistance mutations, with just `r n_katg_greater_2` (`r fmt_pc(n_katg_greater_2/inh_n_uniq_KRM_katg)`) occurring in three or more samples. Of these, `r n_katg_greater_2_ins_del_dup_promoter` were indels, duplications or promoters, leaving `r n_katg_greater_2_non_ins_del_dup_non_promoter` known *katG* mutations which were used in later statistical comparisons to the `r MOST_IMPORTANT_NUMBER` putative resistance mutations **Table S5**.
...
# merge(PRM_summary, dplyr::select(subset(fitness_est, status == "unknown"), change, ddg), 
#       by.x = PRM

PRM_summary$PRM <- paste0(PRM_summary$PRM, " [", PRM_summary$n, "]")
PRM_summary <- select(PRM_summary, Drug, PRM, PRM_one_lin)
PRM_summary <- dplyr::select(PRM_summary, -(Drug))

names(PRM_summary) <- c("Mutation [frequency]", "PRM_one_lin")
...
rare_mutations_table$Change <- gsub("\\*", "\\\\*", rare_mutations_table$Change)
...
PRM_KRM_other <- dplyr::select(PRM_KRM_other, -(Drug))

names(PRM_KRM_other) <- c("Putative resistance mutations* (*katG*)", "Known resistance mutations", "Other mutations", "n")
...
# PLOT 
# x-axis - n sublins
# y-axis - % samples with CM
# Filters - >3 samples, no promoters, PRM no katG


# Get data for both axes - subset and split out haplotypes

# KRM
# KRM <- subset(inh_bin, KRM_bin == "present" & PRM_bin == "absent")
KRM <- subset(inh_bin, KRM_katG_bin == "present" & PRM_bin == "absent")
# KRM <- data.frame(KRM %>% mutate(KRM = strsplit(as.character(KRM), "; ")) %>% unnest(KRM))
KRM <- data.frame(KRM %>% mutate(KRM_katG = strsplit(as.character(KRM_katG), "; ")) %>% unnest(KRM_katG))
# PRM
# Use PRM with no known katG
PRM <- data.frame(inh_4_e_g_table %>% mutate(PRM = strsplit(as.character(PRM), "; ")) %>% unnest(PRM))


# y-axis - % samples with CM

# KRM y-axis
KRM_CM_tab <- pivot_num(KRM, formula(KRM_katG ~ CM_bin))
KRM_CM_tab$KRM_katG <- clean_del_ins_dup(KRM_CM_tab$KRM_katG)
KRM_CM_tab$status <- rep('known', nrow(KRM_CM_tab))

# PRM y-axis

# Split out PRM haplotypes (one row per PRM, duplicating the ID and everything else in the row)

PRM_CM_tab <- pivot_num(PRM, formula(PRM ~ CM_bin))
# Clean up
PRM_CM_tab$PRM <- clean_del_ins_dup(PRM_CM_tab$PRM)
PRM_CM_tab <- sort_pos(PRM_CM_tab, "PRM")
PRM_CM_tab$status <- rep('putative', nrow(PRM_CM_tab))

# Put together and get % for CM
CM_data <- rbind_mod(KRM_CM_tab, PRM_CM_tab, c("mutation", drop_col_name(KRM_CM_tab, "KRM_katG")))
CM_data$CM_pc <- round((CM_data$present / (CM_data$absent + CM_data$present) )*100, 2)
...
# First get n col, then merge in n sublins
KRM_n_tab <- reshape2::dcast(KRM, KRM_katG ~ 'n', value.var = "KRM_katG", fun.aggregate = length)
KRM_sublin_n <- reshape2::dcast(reshape2::dcast(KRM, KRM_katG + sublin ~ 'n',
                                                  value.var = "KRM_katG",
                                                  fun.aggregate = length),
                                  KRM_katG ~ 'n_sublin', value.var = "KRM_katG", fun.aggregate = length)
KRM_sublin_tab <- merge(KRM_n_tab, KRM_sublin_n, by = "KRM_katG", all.x = T, sort = F)
KRM_sublin_tab$KRM_katG <- clean_del_ins_dup(KRM_sublin_tab$KRM_katG)
KRM_sublin_tab$status <- rep("known", nrow(KRM_sublin_tab))


# PRM - same as KRM x-axis

PRM_n <- reshape2::dcast(PRM, PRM ~ 'n', value.var = "PRM", fun.aggregate = length)
PRM_sublin_n <- reshape2::dcast(reshape2::dcast(PRM, PRM + sublin ~ 'n', value.var = "PRM", fun.aggregate = length), 
                                PRM ~ 'n_sublin', value.var = "PRM", fun.aggregate = length)
PRM_sublin_tab <- merge(PRM_n, PRM_sublin_n, by = "PRM", all.x = T, sort = F)
PRM_sublin_tab$status <- rep("putative", nrow(PRM_sublin_tab))

# Put together
sublin_data <- setNames(rbind_force(KRM_sublin_tab, PRM_sublin_tab),
                        c("mutation", "n", "n_sublin", "status"))


# x- and y-axes - PUT TOGETHER

CM_sublin_data <- merge(CM_data, dplyr::select(sublin_data, -(status)), 
                                               by = "mutation", all.x = T, sort = F)
s
# # Apply filters
CM_sublin_data <- subset(CM_sublin_data, n > 2)
CM_sublin_data <- subset(CM_sublin_data, !(grepl("c.", mutation)))

# PRM_med <- median(subset(CM_data, status == 'putative')$CM_pc)
# 
# # Plot each PRM % of samples with a CM with only 
# fitness_plot <- ggplot() + 
#   geom_point(data = subset(CM_data, status == "unknown" | change == "katG-p.Ser315Thr"),
...
max_sublin <- max(CM_sublin_data$n_sublin)
x_ax_A <- c(0:10)
# x_ax_B <- seq(15, max_sublin/2, 5)
# x_ax_C <- seq(max(x_ax_A)+10, max_sublin, 10)
x_ax_B <- seq(max(x_ax_A)+10, max_sublin, 10)
x_ax <- c(x_ax_A, x_ax_B)

max_log_n <- max(log(CM_sublin_data$n))
sz_leg <- round_any(exp(c(2, 4, 6, max_log_n)), 10)

sublin_CM_plot <- ggplot() +
  geom_point(data = CM_sublin_data,
             aes(x = n_sublin, y = CM_pc,fill = status),
             colour="black", pch=21, size = 3)+
  # geom_jitter(data = CM_sublin_data,
  #          aes(x = n_sublin, y = CM_pc,fill = status),
  #          colour="black", pch=21, size = 3)+
  scale_y_continuous(name="% samples with compensatory mutations")+
  theme(aspect.ratio=50/100)+
  geom_text(data=subset(CM_sublin_data, mutation == "katG-p.Ser315Thr"),
            aes(x = n_sublin, y = CM_pc, label = "Ser315Thr"),
            hjust = 1.1)+
  scale_x_log10(breaks = x_ax, name="n sublineages")+
  theme_bw()
...
KRM_katg <- data.frame(KRM_katg %>% group_by(KRM_katG) %>% filter(n() > 2))

# Remove ins del dup
KRM_katg <- subset(KRM_katg, !(grepl("del|dup|ins", KRM))) 
...
# Remove promoters from known 
katg_KRM_CM_pivot$pos <- as.numeric(find_pos(katg_KRM_CM_pivot$KRM_katG))
katg_KRM_CM_pivot <- subset(katg_KRM_CM_pivot, pos > 0)
katg_KRM_CM_pivot <- dplyr::select(katg_KRM_CM_pivot, -(pos))
...
KRM_PRM_CM_pc_tab_plot$status <- ifelse(KRM_PRM_CM_pc_tab_plot$status == "unknown", "putative", KRM_PRM_CM_pc_tab_plot$status)

# Get numbers from table
KRM_PRM_CM_pc_tab_plot_n_KRM <- nrow(subset(KRM_PRM_CM_pc_tab_plot, status == "known"))
KRM_PRM_CM_pc_tab_plot_n_PRM <- nrow(subset(KRM_PRM_CM_pc_tab_plot, status == "putative"))


# Make dup for display
KRM_PRM_CM_pc_tab <- KRM_PRM_CM_pc_tab_plot
KRM_PRM_CM_pc_tab <- dplyr::select(KRM_PRM_CM_pc_tab, status, change, n, KRM_pc, CM_pc, fitness_cost_class)
KRM_PRM_CM_pc_tab$KRM_pc <- round(KRM_PRM_CM_pc_tab$KRM_pc/100, 2)
KRM_PRM_CM_pc_tab$CM_pc <- round(KRM_PRM_CM_pc_tab$CM_pc/100, 2)
...
names(KRM_PRM_CM_pc_tab) <- c("Mutation Status", 
                              "Change", 
                              "N isolates", 
                              "Proportion with resistance mutations present", 
                              "Proportion with compensatory mutations present", 
                              "Proxy fitness cost classification*")
...
# x = CM_pc
# y = KRM_pc
# Groups - known vs putative
# Are the slopes significantly different between the two groups?
KRM_CM_lm <- lm(data = KRM_PRM_CM_pc_tab_plot, KRM_pc ~ CM_pc + status)

# T-test - KRM_pc (KRM co-occurrence) by group known vs unknown
KRM_co_occurrence_pc_t_test <- t.test(KRM_pc/100 ~ status, data = KRM_PRM_CM_pc_tab_plot)

# T-test - CM_pc (CM co-occurrence) by group known vs unknown
CM_co_occurrence_pc_t_test <- t.test(CM_pc/100 ~ status, data = KRM_PRM_CM_pc_tab_plot)
# t.test(CM_pc ~ status, data = subset(KRM_PRM_CM_pc_tab_plot, !(change == "Ser315Thr")))
CM_co_occurrence_pc_t_test_ex_315 <- t.test(CM_pc ~ status, data = subset(KRM_PRM_CM_pc_tab_plot, !(grepl("315", change))))

```

The `r inh_n_uniq_PRM_katg` putative INH-*katG* resistance mutations occurred in multiple lineages (L1-L5) with many showing evidence of convergent evolution (**Figure \@ref(fig:isoniazid-tree)**). The putative resistance mutations occur in similar numbers of sub-lineages and at similar *katG* gene positions compared to known resistance mutations, indicating that they show comparable phylogenetic and gene location characteristics (**Figure \@ref(fig:homoplasy-plot)A**). Due to the multi-drug regimens used for TB treatment, resistance often develops to multiple drugs in a stepwise manner [@Libiseller-Egger2020]. The co-occurrence of the `r inh_n_uniq_PRM_katg` *katG* mutations with other resistance mutations was analysed to characterise the isolate profiles in which putative resistance mutations occur. The `r inh_n_uniq_PRM_katg` *katG* mutations were most frequently found in isolates characterised as `r PRM_DR_max` (`r PRM_DR_max_pc`), but also common representation in pre-MDR-TB (`r PRM_DR_pre_MDR_pc`) and pre-XDR-TB (`r PRM_DR_pre_XDR_pc`). Interestingly, around half (`r PRM_n_samps_inh_co_occur`/`r inh_4_a_c_e_g`; `r PRM_pc_samps_inh_co_occur`) of isolates with any of the `r MOST_IMPORTANT_NUMBER` *katG* mutations, had co-occurrence with others linked to INH, with the *fabG1* -15C>T promoter mutation being the most frequent (`r PRM_n_fabg_15`/`r inh_4_a_c_e_g`; `r PRM_pc_fabg_15`) (**Table S4**). This observation is in stark contrast to the *katG* Ser315Thr mutation, the most prevalent resistance mutation in INH resistant isolates and known to confer a high level of resistance, which only co-occurs with other INH resistance mutations in `r pc_315_KRM_co_occur` of isolates. Of the `r inh_4_a_c_e_g` samples with a putative resistance mutation (**Figure S1**), `r inh_n_samps_PRM_DST` (`r fmt_pc(inh_n_samps_PRM_DST/inh_4_a_c_e_g)`) had an available DST result for INH with `r inh_n_samps_PRM_DST_1` reporting a resistant phenotype leading to a highly significant association of putative drug resistance mutation and DST (P<`r changeSciNot(PRM_DST_p)`). Of those with a resistant DST (n=`r inh_n_samps_PRM_DST_1`), `r PRM_DST_1_no_KRM` (`r fmt_pc(PRM_DST_1_no_KRM/inh_n_samps_PRM_DST_1)`) had no other known mutations that could explain resistance.

Isolates with mutations with a high level of drug resistance tend to have low numbers of co-occurring resistance mutations linked to that resistance. As a proxy for measuring resistance level, we measured the percentages of known and putative resistance mutation samples with co-occurring (non-*katG* - *fabG1*, *inhA*, *kasA*) mutations. Mutations at the *katG* 315 codon position mutation, which are known to confer high resistance [@Coll2018a], had a relatively low proportion of samples with co-occurring resistance mutations, with four out of the five known codon 315 mutations having ~20% or fewer proportion of samples with co-occurring resistance mutations (**Figure \@ref(fig:homoplasy-plot)B**). 

In order to allow for a meaningful comparison between the known and putative mutations, the known mutations were filtered such that they only occur in at least three samples,  those samples have no co-occurring putative resistance mutations, and are only in non-promoter regions, leaving `r KRM_PRM_CM_pc_tab_plot_n_KRM` mutations. The putative resistance mutations were also filtered so that their samples had no co-occurring known *katG* mutations and occurred in at least three samples after applying this filter. This excluded one mutation, Met257Val, leaving `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative resistance mutations (**Table S5**<!-- KRM_PRM_CM_pc_tab_plot-->). 

There was no significant difference between the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known and `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative *katG* resistance mutations in terms of proportions of co-occurrence of other resistance mutations (mean resistance co-occurrence proportion: known mutations `r round(KRM_co_occurrence_pc_t_test$estimate[1], 2)` vs. putative `r round(KRM_co_occurrence_pc_t_test$estimate[2], 2)`; T-test P = `r round(KRM_co_occurrence_pc_t_test$p.value, 2)`).

*Mutation fitness*

Compensatory mutations are known to occur when mutations with high fitness costs occur (e.g. *katG* loss of function). To estimate the fitness impact of the `r inh_n_uniq_PRM_katg` putative resistance mutations we used the co-occurrence with a compensatory mutation as a proxy, and also included the Ser315Thr and other codon 315 mutations for comparison. The proxy fitness cost was discretised into 'low', 'medium' and 'high' categories (see Methods). The *katG* Ser315Thr is known to confer a low fitness cost [@Pym2002], was classified into our 'low' category, and has only ~3% of samples containing the mutation co-occurring with a compensatory mutation. In fact, mutations at the codon 315 position appear to have low fitness cost (**Figure \@ref(fig:homoplasy-plot)B**), where four out of the five known codon 315 resistance mutations were classified into our 'low' fitness cost category.

Overall, compensatory mutations seem to occur in a higher proportion in isolates with our putative `r MOST_IMPORTANT_NUMBER` *katG* resistance mutations (`r inh_4_e`/`r nrow(inh_4_e_g_table)`) compared to Ser315Thr (`r inh_n_samps_ser315_and_CM`/`r fmt(nrow(inh_kat315_no_PRM))`) (Chisq P < 10^-16^), suggesting that on average they incur a greater fitness cost compared to this high frequency global mutation. 

Similarly, after applying the same filters as the resistance level analysis above (**Table S5**<!-- KRM_PRM_CM_pc_tab_plot-->), there was a higher proportion of isolates with a compensatory mutation in those with the `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative mutations compared to those with the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known mutations (proportion with compensation mutation: known `r round(CM_co_occurrence_pc_t_test$estimate[1], 2)` vs. putative `r round(CM_co_occurrence_pc_t_test$estimate[1], 2)`; T-test P = 6×10^-5^). This difference remained statistically significant even when excluding the 315 codon positions (P = 4×10^-4^). 

There appears to be little association between known resistance mutation co-occurrence (resistance level) and proxy fitness cost in both the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known and `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative resistance mutations (linear model P = `r lmp(KRM_CM_lm)`) (**Table S5**<!-- KRM_PRM_CM_pc_tab_plot-->). 

Further, there were no strong differences in terms of resistance mutation co-occurrence (resistance level) between isolates with the known and putative resistance mutations in each of our three fitness cost categories high, medium and low (minimum P = 0.3; **Figure \@ref(fig:homoplasy-plot)B)**. No strong differences in the co-variation between resistance level and fitness cost across known and putative mutations supports the veracity of our putative resistance variants.

```{r protein-structure, echo = F, warning=F}

n_known_dist <- nrow(subset(distances, status == "known"))
n_known_fitness <- nrow(subset(fitness_est, status == "known" & !(is.na(ddg))))

dist_t <- t.test(dist ~ status, data = distances)
fitness_t <- t.test(ddg ~ status, data = fitness_est)


dist_ddg <- merge(KRM_PRM_CM_pc_tab_plot, select(distances, mutation, dist), 
                  by.x = "change", by.y = "mutation", all.x = T, sort = F)
dist_ddg <- merge(dist_ddg, select(fitness_est, change, ddg), 
                  by = "change", all.x = T, sort = F)

max_log_n <- max(log(dist_ddg$n))
sz_leg <- round_any(exp(c(2, 4, 6, max_log_n)), 10)

fitness_dist_boxplot <- ggplot()+
  geom_boxplot(data = dist_ddg, aes(x = fitness_cost_class, y = dist, fill = status),
               position=position_dodge(0.8),
               alpha = 0.7)+
  geom_point(data = dist_ddg,
           aes(x = fitness_cost_class, y = dist, fill = status, size = log(n)),
           colour="black", pch=21,
           position = position_dodge(0.8))+
  scale_size(name   = "n",
             breaks = log(sz_leg),
             labels = sz_leg)+
  geom_label_repel(data = subset(dist_ddg, grepl("315", change)),
                   aes(x = fitness_cost_class, y = dist, label = change),
                   position = position_nudge(x = -0.2),
                   size = 4,
                   box.padding = unit(1, "lines"),
                   point.padding = unit(0.1, "lines"))+
  xlab("Fitness cost")+
  ylab(expression(paste("Distance from heme-binding site (", ring(A), ")")))+
  theme_bw()

fitness_dist_boxplot_file <- "../results/fitness_dist_boxplot.png"
ggsave(plot = fitness_dist_boxplot, file = fitness_dist_boxplot_file, width = 1100/4, height = 700/4, units = "mm")


fitness_ddg_boxplot <- ggplot()+
  geom_boxplot(data = dist_ddg, aes(x = fitness_cost_class, y = ddg, fill = status),
               position=position_dodge(0.8),
               alpha = 0.7)+
  geom_point(data = dist_ddg,
           aes(x = fitness_cost_class, y = ddg, fill = status, size = log(n)),
           colour="black", pch=21,
           position = position_dodge(0.8))+
  scale_size(name   = "n",
             breaks = log(sz_leg),
             labels = sz_leg)+
  geom_label_repel(data = subset(dist_ddg, grepl("315", change)),
                   aes(x = fitness_cost_class, y = ddg, label = change),
                   position = position_nudge(x = -0.2),
                   size = 4,
                   box.padding = unit(1, "lines"),
                   point.padding = unit(0.1, "lines"))+
  xlab("Fitness cost")+
  ylab(expression(paste("Predicted Stability Change (", symbol(Delta), symbol(Delta), "G)")))+
  theme_bw()
fitness_ddg_boxplot_file <- "../results/fitness_ddg_boxplot.png"
ggsave(plot = fitness_ddg_boxplot, file = fitness_ddg_boxplot_file, width = 1100/5, height = 700/5, units = "mm")
...
To explore the functional effects of the `r inh_n_uniq_PRM_katg` putative resistance mutations, *in silico* predictions of effects on the *katG* target protein were assessed (**Figure S3**, **Table \@ref(tab:PRM-summary-table)**). 

The distances from the *katG* heme-binding site, thought to be close to the active INH binding site and crucial to enzymatic activity [@Munir2021], did not differ significantly between `r n_known_dist` known (mutations excluding promoters) and `r MOST_IMPORTANT_NUMBER` putative *katG* resistance mutations (mean distance: known `r dist_t$estimate[1]` A^o^ vs. putative `r dist_t$estimate[2]` A^o^; Wilcoxon P = `r round(dist_t$p.value, 2)`). Mutations further from the INH binding site would be more likely to cause less resistance, therefore these results indicate that our `r inh_n_uniq_PRM_katg` putative resistance mutations are comparable to the known *katG* resistance mutations in terms of resistance level, on average. Contrary to the results comparing the number of samples with compensatory mutations as a proxy of fitness cost, there was no significant difference in protein stability change between the `r n_known_fitness` known resistance mutations and the `r MOST_IMPORTANT_NUMBER` putative resistance mutations (mean $\Delta\Delta$G: known `r fitness_t$estimate[1]` vs. putative `r fitness_t$estimate[2]`; Wilcoxon P = `r round(fitness_t$p.value, 2)`).

## DISCUSSION

Our goal was to identify putative novel mutations underlying resistance to RIF and INH, by finding isolates with established compensatory mutations. No novel *rpoB* gene mutations potentially linked to resistance to RIF were found, but this may be expected since there are limited ways in which the precise machinery of RNA polymerase can change without loss of function. In contrast, many changes in the KatG protein can leave the bacteria largely unaffected. Whilst our analysis focused on `r MOST_IMPORTANT_NUMBER` high quality and frequent putative mutations in *katG*, less common mutations identified in one or two isolates may be of interest, including for functional evaluation and surveillance applications. In lieu of DST testing, we have provided evidence that our `r MOST_IMPORTANT_NUMBER` *katG* putative resistance mutations are causally resistance-conferring. These mutations occur in multiple sub-lineages and independently in the phylogeny, a pattern of convergent evolution that is well established in the known *katG* resistance mutations.

No significant differences were found in the proportion of isolates with (non-*katG*) known resistance mutation co-occurrence (our proxy for resistance level) between the `r KRM_PRM_CM_pc_tab_plot_n_KRM` filtered known resistance mutations and our `r KRM_PRM_CM_pc_tab_plot_n_PRM` filtered putative resistance mutations. In showing a similar pattern of resistance mutation co-occurrence we infer that our putative resistance mutations confer a similar level of resistance as known mutations on average and that our putative resistance mutations are likely to be genuine. 

There was however a difference in the fitness cost between the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known and `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative resistance mutations, with the latter appearing to have on average a higher cost. Whilst known resistance mutations are likely to converge on the most stable protein configurations and hence proliferate, the putative mutations are rarer and have not been associated with drug resistance before. There was no relationship between resistance level (using co-occurrence with other resistance mutations as a proxy) and fitness cost in either the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known or `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative filtered resistance mutations. Again, this similar pattern of variation indicates the veracity of the putative resistance mutations.

The functional impact of the `r MOST_IMPORTANT_NUMBER` putative *katG* mutations is supported by *in silico* protein modelling. Distances from the *katG* heme active binding site to the mutations were similar between known and putative variants, indicating that they are likely confer a similar pattern of resistance. In contrast to the differences between known and putative mutations in their percentages of samples with compensatory mutations, there was no difference in the *in silico* prediction of known and putative protein stability. This result is perhaps surprising given the significant difference between known and putative mutations in terms of the percentages of samples with compensatory mutations. However, the $\Delta\Delta$G measure is only a prediction of stability, and therefore an indirect indication of fitness cost.

In principle, our methods could potentially be applied to any drugs which have a similar compensatory-resistance dynamic. This is especially true for non-essential targets that can exhibit multiple resistance mutations without loss of function, such as *katG*. Compensatory mutations for streptomycin (STM) for example are purported to restore translational accuracy of the ribosome, the target of STM [@Bjorkman1998]. However the ribosome is an essential structure that relies on precise machinery, akin to RNA polymerase. It seems less likely therefore that novel resistance mutations will be found, for the same reasons our methods failed to uncover new resistance mutations for RIF. Similarly, other drugs in which purported compensatory mutations have been found act upon structures intolerant to change - DNA gyrase subunit A (*gyrA* gene) for flouroquinolones, and 16S rRNA of the 30S ribosome subunit (*rrs* gene) for other aminoglycasides such as capreomycin [@Emane2021].  

## CONCLUSIONS

We have presented an approach to identify potential resistance mutations to monitor the development of resistance mechanisms to important first-line isoniazid and rifampicin anti-TB drugs, and therefore MDR-TB. The list of putative resistance mutations can inform functional studies of resistance for their verification, and after validation added to the database of known resistance mutations for surveillance and clinical decision making applications using whole genome and amplicon-based Mtb sequencing.
...
n_known_katg_more_than_2_samps_pos_greater_zero <- length(unique(known_pivot$KRM_katG))


```

```{r homoplasy-plot, echo=FALSE, warning = F, out.width = "2000px", fig.cap=sprintf("**(TOP)** Homoplasy among %s known *katG* and %s putative resistance *katG* mutations. The common *katG* Ser315Thr mutation is highlighted. Mutations occurring in <3 isolates and non-protein coding regions are omitted; **(BOTTOM)** Fitness cost versus percentage of samples with co-occurring known (non-*katG* gene) resistance mutations in %s known and %s putative *katG* resistance mutations.", KRM_PRM_CM_pc_tab_plot_n_KRM, MOST_IMPORTANT_NUMBER, KRM_PRM_CM_pc_tab_plot_n_KRM, MOST_IMPORTANT_NUMBER)}
...
```{r KRM-CM-pc-boxplot, echo=FALSE, warning = F, out.width = "2000px"}

knitr::include_graphics(KRM_CM_PC_boxplot_file)

```

```{r fitness-dist-boxplot, echo=FALSE, warning = F, out.width = "2000px"}

knitr::include_graphics(fitness_dist_boxplot_file)

```

```{r fitness-ddg-boxplot, echo=FALSE, warning = F, out.width = "2000px"}

knitr::include_graphics(fitness_ddg_boxplot_file)
...
knitr::kable(common_KRM_tab, row.names = F, format = "pipe")
...
Mutations in compensatory genes and their frequencies in 32k samples. All mutations occur in >1 lineage.
...
Less frequent mutations (n=`r inh_n_unique_rare_katg`) in *katG* (<3 isolates) in `r inh_5_a` isolates with no known INH resistance mutations, with compensatory mutations but no potential resistance mutation**

```{r rare-mutations-table, echo = F, warning=F}

knitr::kable(rare_mutations_table, row.names = F, format = "pipe")
...
Known resistance and other mutations co-occurring in isolates with the `r MOST_IMPORTANT_NUMBER` putative drug resistance mutations

```{r PRM-KRM-other-table, echo = F, warning=F}

knitr::kable(PRM_KRM_other, row.names = F, format = "pipe")

```

\* `r MOST_IMPORTANT_NUMBER` high quality *katG* mutations

```{r KRM-PRM-CM-pc-tab-numbers, echo = F, warning=F}

n_known_KRM_PRM_CM_tab <- nrow(subset(KRM_PRM_CM_pc_tab_plot, status == "known"))
n_PRM_KRM_PRM_CM_tab <- nrow(subset(KRM_PRM_CM_pc_tab_plot, status == "putative"))

```

**Table S5**

Proportions of co-occurring known and compensatory resistance mutations for the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known and `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative *katG* resistance mutations. Mutations in fewer than three samples are omitted. Co-occurring known resistance mutations are in *fabG1*, *inhA*, and *kasA* genes.

```{r KRM-PRM-CM-pc-tab, echo = F, warning=F}

# % co-occurence with another resistance mutation and % co-occurence with a CM

knitr::kable(KRM_PRM_CM_pc_tab, row.names = F, format = "pipe")

```

\* Based on tertiles of the proportion of compensatory mutations present

## SUPPLEMENTARY FIGURES

**Figure S1**

Analysis strategy and numbers of mutations

```{r samples-trees, echo=FALSE, out.width = "2000px", fig.show='hold'}
...
<!-- Number of samples versus percentage of samples with a compensatory mutation for the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known and `r KRM_PRM_CM_pc_tab_plot_n_KRM` putative katG mutations. Ser315Thr is labeled, and has low fitness cost, and infrequently co-occurs a compensatory mutation. See **Table S5** -->

Number of sublineages versus percentage of samples with a compensatory mutation for the `r KRM_PRM_CM_pc_tab_plot_n_KRM` known and `r KRM_PRM_CM_pc_tab_plot_n_KRM` putative katG mutations. Ser315Thr is labeled, and has low fitness cost, and infrequently co-occurs a compensatory mutation.

```{r sublin-CM-plot, echo = F, warning=F}

# fitness_pc_total_plot_file <- "../results/fitness_pc_total_plot.png"
sublin_CM_plot_file <- "../results/sublin_CM_plot.png"
ggsave(plot = sublin_CM_plot, file = sublin_CM_plot_file, width = 1100/6, height = 700/6, units = "mm")
knitr::include_graphics(sublin_CM_plot_file)
...
```{r protein-model, echo=FALSE, warning = F, out.width = "2000px"}
