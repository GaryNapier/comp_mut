...
Mutations with a high level of resistance have lower proportions of samples with co-occurring resistance mutations. Therefore as a proxy for measuring resistance level, we measured the percentages of known and putative resistance mutation samples with co-occurring (non-*katG* - *fabG1*, *inhA*, *kasA*) mutations. For example, mutations at the *katG* codon 315 position mutation is known to confer high resistance [@Coll2018a], and has a relatively low proportion of samples with co-occurring resistance mutations, with four out of the five known 315 mutations have ~20% or fewer proportion of samples with co-occurring resistance mutations (**Figure S4**).

There was no significant difference between the known and putative resistance mutations in terms of co-occurrence of other resistance mutations (mean resistance co-occurrence among known mutations = 30.2%, putative = 38.7%; p = 0.4, excluding samples with just one or two known *katG* resistance mutations). 
...
The distances from the *katG* heme site, thought to be close to the active INH binding site and crucial to enzymatic activity [@Munir2021], did not differ significantly between the known and putative resistance mutations (mean known = `r dist_t$estimate[1]` A^o^; mean putative = `r dist_t$estimate[2]` A^o^; p = `r round(dist_t$p.value, 2)`).

To infer fitness cost in the putative resistance mutations, the stability of the protein was estimated as the change in Gibbs free energy ($\Delta\Delta$G). There was however, no significant difference between the known resistance mutations and the putative resistance mutations (mean known = `r fitness_t$estimate[1]`; mean putative = `r fitness_t$estimate[2]` p = `r round(fitness_t$p.value, 2)`). 
...
In showing a similar pattern of resistance mutation co-occurrence we infer that our putative resistance mutations confer a similar level of resistance as known mutations on average and that our putative resistance mutations are likely to be genuine. 
