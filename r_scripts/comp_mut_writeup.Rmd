...
CM_pc_box <- ggplot()+
  geom_boxplot(data = KRM_PRM_CM_pc_tab_plot, aes(x = fitness_cost_class, y = CM_pc, fill = status), 
               position=position_dodge(0.8))+
  geom_point(data = KRM_PRM_CM_pc_tab_plot,
           aes(x = fitness_cost_class, y = CM_pc, fill = status, size = log(n)),
           colour="black", pch=21,
           position=position_dodge(0.8))+
  geom_text(data=subset(KRM_PRM_CM_pc_tab_plot, change == "Ser315Thr"),
          aes(x = fitness_cost_class, y = CM_pc, label = "Ser315Thr"), 
          hjust = 1.6, size = 3)+
  xlab("Fitness cost")+
