...
KRM_co_occurrence_pc_t_test$p.value
...
There was no significant difference between the 40 known 
...
and `r MOST_IMPORTANT_NUMBER` putative *katG* resistance mutations in terms of proportions of co-occurrence of other resistance mutations (mean resistance co-occurrence proportion: known mutations `r round(KRM_co_occurrence_pc_t_test$estimate[1], 2)` vs. putative `r round(KRM_co_occurrence_pc_t_test$estimate[2], 2)`; T-test P = `r round(KRM_co_occurrence_pc_t_test$p.value, 2)`).
...
There appears to be little association between known resistance mutation co-occurrence (resistance level) and proxy fitness cost in both the 40 known and `r MOST_IMPORTANT_NUMBER` putative resistance mutations (linear model P = `r lmp(KRM_CM_lm)`). Further, there were no strong differences in terms of resistance mutation co-occurrence (resistance level) between isolates with the known and putative resistance mutations in each of our three fitness cost categories (minimum P > 0.4; **Figure \ref@(fig:PRM-summary-table)B)**. No strong differences in the co-variation between resistance level and fitness cost across known and putative mutations supports the veracity of our putative resistance variants.
