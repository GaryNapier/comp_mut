...
n_katg_less_3 <- nrow(subset(common_KRM_tab, Gene == "katG" & n < 3))
n_katg_less_5 <- nrow(subset(common_KRM_tab, Gene == "katG" & n < 5))
n_rpob_less_5 <- nrow(subset(common_KRM_tab, Gene == "rpoB" & n < 5))
...
Our data contained `r inh_n_uniq_KRM_katg` distinct known *katG* resistance mutations, with just `r inh_n_uniq_KRM_katg - n_katg_less_3` (`r fmt_pc((inh_n_uniq_KRM_katg - n_katg_less_3)/inh_n_uniq_KRM_katg)`) occurring in more than three samples and `r inh_n_uniq_KRM_katg - n_katg_less_5` (`r fmt_pc((inh_n_uniq_KRM_katg - n_katg_less_5)/inh_n_uniq_KRM_katg)`) occurring in more than five samples. The most common mutations underlying INH resistance were `r inh_KRM_top_1` (n=`r inh_KRM_top_1_n`; `r inh_KRM_top_1_pc`), `r inh_KRM_top_2` (n=`r inh_KRM_top_2_n`; `r inh_KRM_top_2_pc`), and `r inh_KRM_top_3` (n=`r inh_KRM_top_3_n`; `r inh_KRM_top_3_pc`). Similarly, for the RIF resistance, the most frequent mutations were `r rif_KRM_top_1` (`r rif_KRM_top_1_pc`), `r rif_KRM_top_2` (`r rif_KRM_top_2_pc`) and `r rif_KRM_top_3` (`r rif_KRM_top_3_pc`) (**Table S1**). 
...
In order to allow for a meaningful comparison between the known and putative mutations, the known mutations were filtered such that they only occur in at least three samples and those samples have no co-occurring putative resistance mutations, leaving `r KRM_PRM_CM_pc_tab_plot_n_KRM` mutations. The putative resistance mutations were also filtered so that their samples had no co-occurring known *katG* mutations and occurred in at least three samples after applying this filter. This excluded one mutation, Met257Val, leaving `r KRM_PRM_CM_pc_tab_plot_n_PRM` putative resistance mutations (**Table S5**<!-- KRM_PRM_CM_pc_tab_plot-->). 
...
The distances from the *katG* heme-binding site, thought to be close to the active INH binding site and crucial to enzymatic activity [@Munir2021], did not differ significantly between `r n_known_dist` known and `r MOST_IMPORTANT_NUMBER` putative *katG* resistance mutations (mean distance: known `r dist_t$estimate[1]` A^o^ vs. putative `r dist_t$estimate[2]` A^o^; Wilcoxon P = `r round(dist_t$p.value, 2)`). Mutations further from the INH binding site would be more likely to cause less resistance, therefore these results indicate that our `r inh_n_uniq_PRM_katg` putative resistance mutations are comparable to the known *katG* resistance mutations in terms of resistance level, on average. 
Contrary to the results comparing the number of samples with compensatory mutations as a proxy of fitness cost, there was no significant difference in protein stability change between the `r inh_n_uniq_KRM_katg` known resistance mutations and the `r MOST_IMPORTANT_NUMBER` putative resistance mutations (mean $\Delta\Delta$G: known `r fitness_t$estimate[1]` vs. putative `r fitness_t$estimate[2]`; Wilcoxon P = `r round(fitness_t$p.value, 2)`).
