...
df <- data.frame(source = rep("security", 3), target = c("Asylum", "Internal", "IT"), value = c(738, 454, 13), id_source = rep(0, 3), id_target = 1:3)




dat <- make_long(df, source, target)

ggplot(dat, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) +
...
**Background:** Resistance to the first-line drugs isoniazid (INH) and rifampicin (RIF) in Mycobacterium tuberculosis (multidrug-resistant TB; MDR-TB) threatens disease control. In the development of M. tuberculosis resistance to anti-TB drugs, genetic mutations often come with a fitness cost. To overcome these fitness costs, M. tuberculosis can develop further, beneficial, compensatory mutations in genes that code for similar proteins. Here we leverage the presence of known resistance a
nd compensatory mutations and phenotypic data to detect the presence of novel mutations occurring in the target genes of INH (e.g., katG) and RIF (e.g., rpoB). 

**Results:** Across XX isolates, there was XXX (XX%) with INH and XXX (XX%) with RIF phenotypic resistance. Known mutations in rpoB (n=XXX/XXX;) and katG (XXX/XXXX) explained most of the resistance…. Known compensatory mutations were identified XXXXXX. By examining isolates with resistance, but known compensatory mutations, we identified XXXXX novel resistance markers where no known resistance ones were present. XXX look at combinations for MDR XXXX

**Conclusions:** Detecting resistance mutations from whole genome sequencing can inform clinical management, but also revealed new markers linked to resistance. We have identified XXX new resistance markers, and after validation, added to list of mutations for genotypic predictions.
