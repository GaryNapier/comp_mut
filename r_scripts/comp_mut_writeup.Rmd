...
inh_KRM_cnt <- data.frame(inh_KRM_dup %>% count(KRM))
...
rif_KRM_cnt <- data.frame(rif_KRM_dup %>% count(KRM))
...
fitness_est_file <- paste0(results_path, "fitness_estimate_results.txt")
...
The distances from the *katG* heme site, thought to be close to the active INH binding site, did not differ significantly between the known and putative resistance mutations (mean known = `r dist_t$estimate[1]` $\A^\circ$; mean putative = `r dist_t$estimate[2]` $\A^\circ$; p = `r dist_t$p.value`).


Fitness cost.. 


To infer fitness cost in the putative resistance mutations, the stability of the protein was estimated as the change in Gibbs free energy ($\Delta\Delta$G). 
...
There was no relationship between resistance level and fitness cost in either the known resistance mutations or putative resistance mutations. Again, this similar pattern of variation indicates the veracity of the putative resistance mutations. 
	...
	- Protein modelling
		- PRMs show no sig diff to the KRMs in their distance from the heme site, indicating that they likely confer a similar pattern of resistance.
		- PRMs and KRMs differ in stability (fitness cost)?
...
Drug resistance types and lineages were predicted *in silico* with TB-profiler (v4.2.0) [@Phelan2019] [@Coll2015]. TB-profiler also used identified all known drug-resistance mutations and all other mutations in relevant genes for finding compensatory mutations and putative novel resistance mutations. 
