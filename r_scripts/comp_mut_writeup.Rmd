...
KRM_PRM_CM_pc_tab$fitness_cost_class <- factor(KRM_PRM_CM_pc_tab$fitness_cost_class, levels = c("high", "medium", "low"))
...
KRM_PRM_CM_pc_plot <- ggplot()+
  geom_point(data = KRM_PRM_CM_pc_tab_plot, aes(x = KRM_pc, y = CM_pc, fill = status, size = log(n)), colour="black", pch=21)+
  ...
  geom_text(data=subset(KRM_PRM_CM_pc_tab_plot, change == "Ser315Thr"),
            aes(x = KRM_pc, y = CM_pc, label = "Ser315Thr"), 
            hjust = -0.2, size = 3)+
  xlab("% samples with known resistance mutation present (in fabG1, inhA or kasA)")+
  ylab("% samples with compensatory mutation present")+
  theme_bw()
...
KRM_pc_box <- ggplot()+
  geom_boxplot(data = KRM_PRM_CM_pc_tab_plot, aes(x = resistance_level, y = KRM_pc, fill = status), 
               position=position_dodge(0.8))+
  geom_point(data = KRM_PRM_CM_pc_tab_plot,
             aes(x = resistance_level, y = KRM_pc, fill = status, size = log(n)),
             colour="black", pch=21,
             position=position_dodge(0.8))+
  geom_text(data=subset(KRM_PRM_CM_pc_tab_plot, change == "Ser315Thr"),
          aes(x = resistance_level, y = KRM_pc, label = "Ser315Thr"), 
          hjust = 1,
          vjust = -1.4,
          size = 3)+
...
CM_pc_box <- ggplot()+
  ...
  geom_point(data = KRM_PRM_CM_pc_tab_plot,
           ...
           position=position_dodge(0.8))+
  geom_text(data=subset(KRM_PRM_CM_pc_tab_plot, change == "Ser315Thr"),
          aes(x = fitness_cost_class, y = KRM_pc, label = "Ser315Thr"), 
          hjust = 1.6, size = 3)+
  xlab("Fitness cost")+
  ylab("% samples with resistance")+
  theme_bw()


ggplot()+
  geom_boxplot(data = KRM_PRM_CM_pc_tab_plot, aes(x = CM_pc, y = resistance_level, fill = status), 
               position=position_dodge(0.8))+
  # geom_point(data = KRM_PRM_CM_pc_tab_plot,
  #          aes(x = resistance_level, y = CM_pc, fill = status, size = log(n)),
  #          colour="black", pch=21,
  #          position=position_dodge(0.8))+
  geom_text(data=subset(KRM_PRM_CM_pc_tab_plot, change == "Ser315Thr"),
          aes(x = CM_pc, y = resistance_level, label = "Ser315Thr"), 
          hjust = 1.6, size = 3)+
  xlab("Resistance level")+
  ylab("% samples with co-occurring compensatory mutations")+
  theme_bw()


ggplot()+
  geom_boxplot(data = subset(KRM_PRM_CM_pc_tab_plot, !(change == "Ser315")), aes(x = fitness_cost_class, y = n, fill = status), 
               position=position_dodge(0.8))+
  ylim(c(1, 30))+
  # geom_point(data = KRM_PRM_CM_pc_tab_plot,
  #          aes(x = resistance_level, y = n, fill = status, size = log(n)),
  #          colour="black", pch=21,
  #          position=position_dodge(0.8))+
  # geom_text(data=subset(KRM_PRM_CM_pc_tab_plot, change == "Ser315Thr"),
  #         aes(x = fitness_cost_class, y = log10(n), label = "Ser315Thr"), 
  #         hjust = 1.6, size = 3)+
  xlab("fitness level")+
  ylab("n")+
  theme_bw()
...
# Pull out legend
leg <- g_legend(KRM_pc_box)

KRM_CM_PC_boxplots <- grid.arrange(arrangeGrob(KRM_pc_box+theme(legend.position="none"), 
                                   CM_pc_box+theme(legend.position="none"),
                                   ncol=2), 
                                   leg, nrow=1, widths = c(6, 1))

ggsave("../results/KRM_CM_PC_boxplots.png", KRM_CM_PC_boxplots, width = 1100/5, height = 700/5, units = "mm")
...
Compensatory mutations are known to occur when mutations with high fitness-costs occur (e.g. *katG* loss of function). To estimate the fitness impact of the `r inh_n_uniq_PRM_katg` putative resistance mutations we used the co-occurrence with a compensatory mutation as a proxy, comparing to *katG* Ser315Thr which has reported to confer a low fitness cost. Compensatory mutations appear to occur in a higher proportion in samples with putative resistance mutations (`r inh_4_e`/`r nrow(inh_4_e_g_table)`) compared to Ser315Thr (`r inh_n_samps_ser315_and_CM`/`r fmt(nrow(inh_kat315_no_PRM))`) (Chisq p < 10-^16^), suggesting that on average they incur a greater fitness cost. 





<!-- **Table S4** shows the percentages of known resistance mutations (among genes *fabG1*, *inhA*, *kasA*) and compensatory mutations co-occuring with known resistance mutations and our list of PRMs.  -->
...
That no RIF *rpoB* potential resistance mutations were found is unsurprising, since there are few ways in which the precise machinery of RNA polymerase can change without loss of function. In contrast, *katG* can change in many ways while the bacteria is largely unaffected. The 'rare' mutations could be considered potential resistance mutations with a larger sample size. These mutations were discarded because they only appeared in one or two samples.

Further research could explore *in silico* prediction of resistance mechanisms in the relevant target proteins, i.e. modeling of how the mutations manifest in the final protein and how drug binding is thereby affected.
...
```{r KRM-CM-pc-plot, echo=FALSE, warning = F, out.width = "2000px", fig.cap="Percentages of samples with known and putative *katG* resistance mutations with co-occurring known resistance mutations (left) and compensatory mutations (right). The known resistance mutation Ser315Thr is highlighted. This mutation conferrs a high level of resistance to INH, hence its samples co-occur with relatively few other (non-*katG*) resistance mutations (~15%). Ser315Thr also incurrs little fitness cost, therefore a low percentage of its samples also have compensatory mutations (~3%). The three categories of high medium and low resistance/fitness are based on tertiles of the respective percentages, except for the 'high' category of resistance level, which is based on the ~15% Ser315Thr resistance mutation co-occurrence. Our putative resistance mutations show similar distributions as known resistance mutations in each category of resistance level and fitness cost. See **Table S4**. Mutations in fewer than three samples are omitted."}

# knitr::include_graphics("../results/KRM_PRM_CM_pc_plot.png")

knitr::include_graphics("../results/KRM_CM_PC_boxplots.png")
