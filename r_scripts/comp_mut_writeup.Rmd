...
# Filter?
PRM <- data.frame(PRM %>% group_by(PRM) %>% filter(n() > 4))
...
KRM_PRM_CM_pc_tab <- select(KRM_PRM_CM_pc_tab, status, change, KRM_pc, resistance_level, CM_pc, fitness_cost_class)
names(KRM_PRM_CM_pc_tab) <- c("Status", "Change", "% known resistance mutations", "Resistance level", "% compensatory mutations", "Fitness cost")
...
```{r homoplasy-plot, echo=FALSE, warning = F, out.width = "2000px", fig.cap="Homoplasy among known and putative ('unknown') resistance mutations in *katG*, as indicated by number of sublineages in which mutations occur (y-axis). Ser315Thr is highlighted."}
