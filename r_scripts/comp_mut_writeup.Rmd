...
rare_mutations_table <- cbind(Drug = c(rep("isoniazid", nrow(rare_katgs)), 
...
inh_n_samps_CM_and_no_KRM <- nrow(subset(inh_bin, KRM_katG_bin == "absent" & CM_bin == "present"))
# rif_n_samps_CM_and_no_KRM <- nrow(subset(rif_bin, !(is.na(CM)) & is.na(KRM)))
rif_n_samps_CM_and_no_KRM <- nrow(subset(rif_bin, KRM_katG_bin = "absent" & CM_bin == "present"))
...
# total_samps
# # 2
# nrow(subset(rif_bin, KRM_rpoB_bin == "present"))
# nrow(subset(rif_bin, KRM_rpoB_bin == "absent"))
# # 3
# nrow(subset(rif_bin, KRM_rpoB_bin == "present" &
#          CM_bin == "present"))
# nrow(subset(rif_bin, KRM_rpoB_bin == "present" &
...
# INH
# a
inh_4_a <- subset(inh_bin, KRM_katG_bin == "present" &
         CM_bin == "present" &
         PRM_bin == "present")$wgs_id
# b
inh_4_b <- subset(inh_bin, KRM_katG_bin == "present" &
         CM_bin == "present" &
         PRM_bin == "absent")$wgs_id
# c
inh_4_c <- subset(inh_bin, KRM_katG_bin == "present" &
         CM_bin == "absent" &
         PRM_bin == "present")$wgs_id
# d
inh_4_d <- subset(inh_bin, KRM_katG_bin == "present" &
         CM_bin == "absent" &
         PRM_bin == "absent")$wgs_id
# e
inh_4_e <- subset(inh_bin, KRM_katG_bin == "absent" &
         CM_bin == "present" &
         PRM_bin == "present")$wgs_id
# f
inh_4_f <- subset(inh_bin, KRM_katG_bin == "absent" &
         CM_bin == "present" &
         PRM_bin == "absent")$wgs_id
# g
inh_4_g <- subset(inh_bin, KRM_katG_bin == "absent" &
         CM_bin == "absent" &
         PRM_bin == "present")$wgs_id
# h
inh_4_h <- subset(inh_bin, KRM_katG_bin == "absent" &
         CM_bin == "absent" &
         PRM_bin == "absent")$wgs_id

samps_list <- list(inh_4_a, inh_4_b, inh_4_c, inh_4_d, inh_4_e, inh_4_f, inh_4_g, inh_4_h)
for(i in seq(letters[1:8])){
  write.table(samps_list[[i]], file = paste0("../results/INH_", letters[i], ".txt"),
...
# RIF
# a
rif_4_a <- subset(rif_bin, KRM_rpoB_bin == "present" &
         CM_bin == "present" &
         PRM_bin == "present")$wgs_id
# b
rif_4_b <- subset(rif_bin, KRM_rpoB_bin == "present" &
         CM_bin == "present" &
         PRM_bin == "absent")$wgs_id
# c
rif_4_c <- subset(rif_bin, KRM_rpoB_bin == "present" &
         CM_bin == "absent" &
         PRM_bin == "present")$wgs_id
# d
rif_4_d <- subset(rif_bin, KRM_rpoB_bin == "present" &
         CM_bin == "absent" &
         PRM_bin == "absent")$wgs_id
# e
rif_4_e <- subset(rif_bin, KRM_rpoB_bin == "absent" &
         CM_bin == "present" &
         PRM_bin == "present")$wgs_id
# f
rif_4_f <- subset(rif_bin, KRM_rpoB_bin == "absent" &
         CM_bin == "present" &
         PRM_bin == "absent")$wgs_id
# g
rif_4_g <- subset(rif_bin, KRM_rpoB_bin == "absent" &
         CM_bin == "absent" &
         PRM_bin == "present")$wgs_id
# h
rif_4_h <- subset(rif_bin, KRM_rpoB_bin == "absent" &
         CM_bin == "absent" &
         PRM_bin == "absent")$wgs_id

samps_list <- list(rif_4_a, rif_4_b, rif_4_c, rif_4_d, rif_4_e, rif_4_f, rif_4_g, rif_4_h)
for(i in seq(letters[1:8])){
  write.table(samps_list[[i]], file = paste0("../results/RIF_", letters[i], ".txt"),
              quote = F, col.names = F, row.names = F)
}

# 5
# a
# subset(rif_bin, KRM_rpoB_bin == "absent" &
#          CM_bin == "present" &
#          PRM_bin == "absent" & 
#          !(is.na(other_rpoB)) )[c("wgs_id", "other_rpoB")]
...
inh_PRM_no_katG_KRM <- subset(inh_bin, PRM_bin == "present" & !(grepl("katG", KRM)))
inh_PRM_no_katG_KRM_pivot <- pivot(inh_PRM_no_katG_KRM, formula(CM_bin ~ PRM_bin))
...
y <- table(inh_PRM_no_katG_KRM$CM_bin, inh_PRM_no_katG_KRM$PRM_bin)
tab <- cbind(x, y)
fitness_x_sq <- print_chisq(chisq.test(tab), rnd = rnd)
...
KRM_tab$prop <- KRM_tab$present/KRM_tab$absent
...
PRM <- data.frame(inh_PRM_no_katG_KRM %>% 
                  ...
                  %>% unnest(PRM))
PRM_tab <- pivot_num(PRM, formula(PRM ~ CM_bin))
PRM_tab$prop <- PRM_tab$present/PRM_tab$absent
PRM_tab$prop <- ifelse(is.infinite(PRM_tab$prop), 1, PRM_tab$prop)
PRM_med <- median(PRM_tab$prop)
# Clean up
...
fitness_plot <- ggplot() + geom_point(data = KRM_PRM_tab, 
                                      aes(x = factor(change, level = change), y = prop, 
                                          colour = status), size = 3)
fitness_plot <- fitness_plot + scale_x_discrete(name="") + 
  scale_y_continuous(name="proportion of samples with CM/no CM")
...
inh_PRM_no_katG_KRM_pivot <- pivot(inh_PRM_no_katG_KRM, formula(KRM_bin ~ PRM_bin))

resistance_table <- merge(inh_kat315_multi_KRM_no_PRM_pivot, inh_PRM_no_katG_KRM_pivot, 
                          by.x = "Ser315Thr_bin", by.y = "KRM_bin", sort = F)

names(resistance_table) <- c("KRM", "Ser315Thr", "PRM")

# Do Chi sq
tab <- cbind(table(inh_kat315_multi_KRM_no_PRM$Ser315Thr_bin, inh_kat315_multi_KRM_no_PRM$KRM_bin), table(inh_PRM_no_katG_KRM$KRM_bin, inh_PRM_no_katG_KRM$PRM_bin))
resistance_x_sq <- print_chisq(chisq.test(tab), rnd = rnd)
...
cap <- "Mutations in *katG* (INH) and *rpoB* (RIF) from samples with no KRM, with CM but no potential resistance mutation by the criteria outlined in *Methods*. These mutations only occur in one or two samples, hence were excluded as PRM"
...
```{r fitness-plot, echo = F, warning=F, fig.cap="Proportions of samples with and without compensatory mutations among the potential resistance mutations. *katG*-Ser315Thr is shown for comparison. This mutation incurs low fitness cost, hence does not generally co-occur in samples with a compensatory mutation. Red line - median proportion for potential resistance mutations."}
...
```{r samples-breakdown, echo=FALSE, out.width = "2000px", fig.show='hold', fig.cap="Breakdown of sample numbers by presence of mutation type for INH (A) and RIF (B)" }

knitr::include_graphics("../results/numbers.png")
...
ahpc_katg_links <- ggplot(df, aes(x = x, next_x = next_x, node = node, next_node = next_node, label = node)) +
  coord_flip()+
  geom_sankey(flow.alpha = .6, fill = 'lightblue', na.rm = T) +
  geom_sankey_text(size = 2.25, color = "black", angle = 45, width = 1, na.rm = T) +
  theme_minimal()+
  labs(x = NULL)+
...
ahpc_katg_links_file <- "../results/ahpc_katg_links.png"

ggsave(ahpc_katg_links, file = ahpc_katg_links_file)

knitr::include_graphics(ahpc_katg_links_file)
