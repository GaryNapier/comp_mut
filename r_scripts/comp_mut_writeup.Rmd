...
PRM_summary$PRM_one_lin <- str_extract(PRM_summary$PRM, paste0(PRMs_one_lin, collapse = "|"))
...
PRM_DR_max_pc <- fmt_pc(PRM_DR_pivot[which.max(PRM_DR_pivot$n), "n"]/inh_n_samps_PRM)
PRM_DR_pre_MDR_pc <- fmt_pc(PRM_DR_pivot[PRM_DR_pivot$drtype == "Pre-MDR-TB", "n"]/inh_n_samps_PRM)
PRM_DR_pre_XDR_pc <- fmt_pc(PRM_DR_pivot[PRM_DR_pivot$drtype == "Pre-XDR-TB", "n"]/inh_n_samps_PRM)
PRM_DR_XDR_pc <- fmt_pc(PRM_DR_pivot[PRM_DR_pivot$drtype == "XDR-TB", "n"]/inh_n_samps_PRM, digits = 2)
PRM_DR_sens_pc <- fmt_pc(PRM_DR_pivot[PRM_DR_pivot$drtype == "Sensitive", "n"]/inh_n_samps_PRM, digits = 2)
PRM_DR_other_pc <- fmt_pc(PRM_DR_pivot[PRM_DR_pivot$drtype == "Other", "n"]/inh_n_samps_PRM, digits = 2)
...
PRM_pc_samps_inh_co_occur <- fmt_pc(sum(subset(PRM_KRM_other, !(is.na(KRM)))$n)/inh_n_samps_PRM)
PRM_pc_fabg_15 <- fmt_pc(sum(subset(PRM_KRM_other, KRM == "fabG1-c.-15C>T")$n)/inh_n_samps_PRM)
...
x <- chisq.test(table(PRM_DST$dst))

x$p.value

```

Of the `r inh_4_a_c_e_g` samples with a PRM, `r inh_n_samps_PRM_DST` had an available DST result for isoniazid with `r inh_n_samps_PRM_DST_1` reporting a resistant phenotype leading to a highly significant association of PRMs and DST (p<XXXX, Table S2).
