...
known_CM_KRM_lm <- lm(data = subset(KRM_PRM_CM_pc_tab_plot, status == "known"), CM_pc ~ KRM_pc)
unknown_CM_KRM_lm <- lm(data = subset(KRM_PRM_CM_pc_tab_plot, status == "unknown"), CM_pc ~ KRM_pc)

```

Compensatory mutations are known to occur when mutations with high fitness-costs occur (e.g. *katG* loss of function). 




To estimate the fitness impact of the `r inh_n_uniq_PRM_katg` putative resistance mutations we used the co-occurrence with a compensatory mutation as a proxy, comparing to *katG* Ser315Thr which has reported to confer a low fitness cost (~3% of samples co-occur with compensatory mutations). 
...
Compensatory mutations appear to occur in a higher proportion in samples with putative resistance mutations (`r inh_4_e`/`r nrow(inh_4_e_g_table)`) compared to Ser315Thr (`r inh_n_samps_ser315_and_CM`/`r fmt(nrow(inh_kat315_no_PRM))`) (Chisq p < 10-^16^), suggesting that on average they incur a greater fitness cost. 





There appears to be little relationship between known resistance mutation co-occurrence and fitness cost in both known (p = `r lmp(known_CM_KRM_lm)`) and unknown (p = `r lmp(unknown_CM_KRM_lm)`) resistance mutations. 




<!-- Mutation Ser315Thr is also known to confer high resistance to INH, and so co-occurs with relatively few other INH (non-*katG*) resistance mutations (~15%). Resistance mutations appear to occur in a higher proportion in samples with putative resistance mutations -->
...
<!-- that there were little differences between our putative INH resistance mutations and known INH resistance mutations in terms of overall resistance level and -->
