...
KRM_PRM_CM_pc_tab_plot <- dplyr::select(KRM_PRM_CM_pc_tab_plot, status, change, n, KRM_pc, CM_pc, fitness_cost_class)
...
There appears to be little association between known resistance mutation co-occurrence (resistance level) and proxy fitness cost in both the 40 known and `r MOST_IMPORTANT_NUMBER` putative resistance mutations (linear model P = `r lmp(KRM_CM_lm)`). Further, there were no strong differences in terms of resistance mutation co-occurrence (resistance level) between isolates with the known and putative resistance mutations in each of our three fitness cost categories (minimum P > 0.4; **Figure \ref@(fig:fitness-pc-plot)B)**. No strong differences in the co-variation between resistance level and fitness cost across known and putative mutations supports the veracity of our putative resistance variants.
...
cap <- "Common established resistance mutations for INH and RIF*. Mutations occurring in <5 isolates in the 32k dataset are omitted."
...
cap <- "Mutations in compensatory genes and their frequencies in 32k samples. All mutations occur in >1 lineage"
...
cap <- sprintf("Less frequent mutations (n=%s) in katG (<3 isolates) in %s isolates with no known INH resistance mutations, with compensatory mutations but no potential resistance mutation**", inh_n_unique_rare_katg, inh_5_a)
...
cap <- sprintf("The 40 known resistance and other mutations co-occurring in isolates with the %s putative drug resistance mutations", MOST_IMPORTANT_NUMBER)
...
cap <- "Proportions of co-occurring known and compensatory resistance mutations for the 40 known and 31 putative *katG* resistance mutations. Mutations in fewer than three samples are omitted. Co-occurring known resistance mutations are in *fabG1*, *inhA*, and *kasA* genes."
...
```{r fitness-pc-plot, echo = F, warning=F, fig.cap="Number of samples versus percentage of samples with a compensatory mutation for the 40 known and 31 putative katG mutations. Ser315Thr is labeled, and has low fitness cost, and infrequently co-occurs a compensatory mutation."}
