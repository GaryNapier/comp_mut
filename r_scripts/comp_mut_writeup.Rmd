...
**Conclusions:** Detecting resistance mutations from whole genome sequencing can inform clinical management, but also revealed new variants linked to resistance. We have identified new resistance markers and, after validation, can be used for genotypic drug-resistance profiling.
...
The TB-profiler platform [@Phelan2019] uses 2,300 mutations across 35 loci to profile Mtb resistance across 21 anti-TB drugs, including RIF and INH. However, the full repertoire of resistance mutations, including for MDR-TB is not fully characterised. The accompanying TB-profiler database consists of ~32k isolates has whole genome sequence and drug susceptibility test (DST) phenotypic data, with genotypic profiles. Here, by investigating those samples with compensatory mutations but no known resistance mutations, we attempt to identify the presence of novel mutations linked to genes for INH and RIF. Further, we attempt to understand the patterns of co-existence between resistance and compensatory mutations in relation to INH and RIF drug resistance. 
...
A total of `r fmt(total_samps)` *M. tuberculosis* isolates with whole genome sequencing and DST data were analysed, and encompassed all major lineages (L4 51.1%, L2 25.3%, L3 11.5%, L1 9.7%) (**Table \@ref(tab:table-1-display)**). For DST data, `r fmt(inh_n_samps_DST_res)` (`r fmt_pc(inh_n_samps_DST_res/inh_n_samps_dst, digits = 3)`) and `r fmt(rif_n_samps_DST_res)` (`r fmt_pc(rif_n_samps_DST_res/rif_n_samps_dst, digits = 3)`) were resistant to INH and RIF, respectively. Genotypic resistance prediction using TB-profiler software inferred that `r fmt(inh_n_samps_TBP_res)` (`r fmt_pc(inh_n_samps_TBP_res/total_samps, digits = 3)`) and `r fmt(rif_n_samps_TBP_res)` (`r fmt_pc(rif_n_samps_TBP_res/total_samps, digits = 3)`) isolates were resistant to INH and RIF, respectively, with `r fmt(n_samps_MDR)` (`r fmt_pc(n_samps_MDR/total_samps, digits = 3)`) isolates classified as MDR-TB (**Table \@ref(tab:table-1-display)**). 
...
The most common mutations underlying INH resistance were `r inh_KRM_top_1` (`r inh_KRM_top_1_pc`), `r inh_KRM_top_2` (`r inh_KRM_top_2_pc`), and `r inh_KRM_top_3` (`r inh_KRM_top_3_pc`). Similarly, for the RIF resistance, the most frequent mutations were `r rif_KRM_top_1` (`r rif_KRM_top_1_pc`), `r rif_KRM_top_2` (`r rif_KRM_top_2_pc`) and `r rif_KRM_top_3` (`r rif_KRM_top_3_pc`) (**Table S5**). 
...
To characterise putative novel resistance mutations, we considered samples which had a compensatory mutation, but no known resistance mutation. A manually curated list of compensatory mutations was generated from the literature (**Table S1**). This list includes mutations in *ahpC* (`r length(inh_CM_uniq)`), *rpoC* (`r length(rif_rpoC)`), and rpoA (`r length(rif_rpoA)`), where the *ahpC* mutations also include novel markers which had strong evidence for compensatory effects through convergent evolution, co-occurrence with loss of function mutations in *katG* as well as association with INH resistant isolates. The number of occurrences of the compensatory mutations within the 32k isolates varied RIF (*rpoC*/*A* range: 5 – 427) and INH (*ahpC* range: 3 – 97). No isolate had more than one compensatory mutation for RIF or INH, and across MDR-TB.
...
Of the `r inh_3_c` isolates that had a compensatory mutation, `r inh_4_f` (`r fmt_pc(inh_4_f/inh_3_c)`) did not have an established resistance mutation, but `r inh_5_a` (/`r inh_4_f`; `r fmt_pc(inh_5_a/inh_4_f)`) were found to have a rare *katG* mutation that did not pass the minimum sample cut-off (>=3) used to define putative resistance mutations (**Table S2**). These rare mutations could also potentially explain the acquisition of a compensatory mutation but were not analysed further. Using this approach, our analysis reduced the number to `r inh_n_uniq_PRM_katg` unique mutations present in `r inh_4_e` isolates, with strong evidence of being putative novel resistance mutations (**Table \@ref(tab:PRM-summary-table)**). These `r inh_n_uniq_PRM_katg` mutations were also found in `r inh_4_c_g` isolates without a compensatory mutation, leading to a total of `r inh_4_a_c_e_g` isolates which had a putative resistance mutation (**Figure S1**).

<!-- A small number of rare known resistance mutations (n=5), occurring in just 3-7 samples, were selected for sensitivity analysis to verify that the search strategy could find rare resistance mutations. Upon re-analysis with the selected mutations taken out of the known resistance list, all mutations were subsequently classified as putative resistance mutations, highlighting the power of this approach.  -->
...
The `r inh_n_uniq_PRM_katg` mutations occurred in multiple lineages (L1-L5) with many showing evidence of convergent evolution (**Figure \@ref(fig:isoniazid-tree)**). The putative resistance mutations occur in similar numbers of sub-lineages and at similar *katG* gene positions compared to known resistance mutations, indicating that they show comparable patterns of resistance level, independent of lineage effects (**Figure S3**). Due to the multi-drug regimens used for TB treatment, resistance often develops to multiple drugs in a stepwise manner. The co-occurrence of the `r inh_n_uniq_PRM_katg` mutations with other resistance mutations was analysed to characterise the isolate profiles in which putative resistance mutations occur. The `r inh_n_uniq_PRM_katg` mutations were most frequently found in isolates characterised as `r PRM_DR_max` (`r PRM_DR_max_pc`), but also common representation in pre-MDR-TB (`r PRM_DR_pre_MDR_pc`) and pre-XDR-TB (`r PRM_DR_pre_XDR_pc`). Interestingly, around half (`r PRM_pc_samps_inh_co_occur`) of samples co-occurred with other INH mutations, with the *fabG1* -15C>T promoter mutation being the most frequent (`r PRM_pc_fabg_15`) (**Table S3**) <!--(**Table \@ref(tab:PRM-KRM-other-table)**)-->. This is in stark contrast to the *katG* Ser315Thr mutation, the most prevalent resistance mutation in INH resistant isolates and known to confer a high level of resistance, which only co-occurs with other INH resistance mutations in `r pc_315_KRM_co_occur` of isolates. Of the `r inh_4_a_c_e_g` samples with a putative resistance mutation (**Figure S1**), `r inh_n_samps_PRM_DST` (`r fmt_pc(inh_n_samps_PRM_DST/inh_4_a_c_e_g)`) had an available DST result for INH with `r inh_n_samps_PRM_DST_1` reporting a resistant phenotype leading to a highly significant association of putative drug resistance mutation and DST (p<`r changeSciNot(PRM_DST_p)`). Of those with a resistant DST (n=`r inh_n_samps_PRM_DST_1`), `r PRM_DST_1_no_KRM` (`r fmt_pc(PRM_DST_1_no_KRM/inh_n_samps_PRM_DST_1)`) had no other known mutations that could explain resistance.

###

Mutations with a high level of resistance have lower proportions of samples with co-occurring resistance mutations.
Therefore as a proxy for measuring resistance level, we measured the percentages of known and putative resistance mutation samples with co-occurring (non-*katG* - *fabG1*, *inhA*, *kasA*) mutations. 
	
For example, mutations at the *katG* 315 position confer high resistance, and have relatively low proportions of samples with co-occurring resistance mutations. In **Figure S4**, four out of the five known 315 mutations have ~20% or fewer proportion of samples with co-occurring resistance mutations.

Our analyses showed that there was no significant difference between the known and putative resistance mutations in terms of co-occurrence of other resistance mutations (mean resistance co-occurrence among known mutations = 30.2%, putative = 38.7%; p = 0.4, excluding samples with just one or two known *katG* resistance mutations). In showing a similar pattern of resistance mutation co-occurrence we infer that our putative resistance mutations confer a similar level of resistance as known mutations on average and that our putative resistance mutations are likely to be genuine. 
...
fitness_pc_total_plot <- fitness_pc_total_plot + geom_text(data=subset(KRM_PRM_tab, change == "katG-p.Ser315Thr"),
                                                           ...
                                                           hjust = 1.1)
fitness_pc_total_plot <- fitness_pc_total_plot + coord_trans(x = 'log')
...
# Get percentages
katg_KRM_CM_pivot$KRM_pc <- (katg_KRM_CM_pivot$KRM_present / (katg_KRM_CM_pivot$KRM_absent + katg_KRM_CM_pivot$KRM_present))*100
katg_KRM_CM_pivot$CM_pc <- (katg_KRM_CM_pivot$CM_present / (katg_KRM_CM_pivot$CM_absent + katg_KRM_CM_pivot$CM_present))*100
...
PRM_KRM_CM_pivot$KRM_pc <- (PRM_KRM_CM_pivot$KRM_present / (PRM_KRM_CM_pivot$KRM_absent + PRM_KRM_CM_pivot$KRM_present))*100
PRM_KRM_CM_pivot$CM_pc <- (PRM_KRM_CM_pivot$CM_present / (PRM_KRM_CM_pivot$CM_absent + PRM_KRM_CM_pivot$CM_present))*100
...
KRM_PRM_CM_pc_tab <- select(KRM_PRM_CM_pc_tab, 
                            status, change, 
                            KRM_absent, KRM_present, KRM_pc, resistance_level,
...
# Make duplicate before renaming 
...
names(KRM_PRM_CM_pc_tab) <- c("Status", "Change", 
                              "n samples known resistance mutations absent", "n samples known resistance mutations present", "known resistance mutations %", "Resistance level", 
                              "n samples known compensatory mutations absent", "n samples known compensatory mutations present", "compensatory mutations %", "Fitness cost")
...
Compensatory mutations are known to occur when mutations with high fitness-costs occur (e.g. *katG* loss of function). To estimate the fitness impact of the `r inh_n_uniq_PRM_katg` putative resistance mutations we used the co-occurrence with a compensatory mutation as a proxy, comparing to *katG* Ser315Thr which has reported to confer a low fitness cost. Approximately ~3% of samples co-occur with compensatory mutations. Indeed, mutations at the 315 position in general appear to have low fitness cost - **Figure S4** shows four out of the five known 315 resistance mutations appearing in our 'low' fitness cost category.

###
Compensatory mutations appear to occur in a higher proportion in samples with putative resistance mutations (`r inh_4_e`/`r nrow(inh_4_e_g_table)`) compared to Ser315Thr (`r inh_n_samps_ser315_and_CM`/`r fmt(nrow(inh_kat315_no_PRM))`) (Chisq p < 10-^16^), suggesting that on average they incur a greater fitness cost compared to this mutation. Comparing the putative resistance mutations to all the known resistance mutations (excluding samples with just one or two known resistance mutations), there was a significant difference in the percentage of samples with co-occurring compensatory mutations (mean known resistance mutations = 19.6%, putative = 51.5%, p = 3×10^-5^), indicating that our putative resistance mutations on average incurr a higher fitness cost. This difference remained significant even when excluding the 315 positions (p = 2×10^-4^).
...
The distances from the *katG* heme site, thought to be close to the active INH binding site, did not differ significantly between the known and putative resistance mutations (mean known = `r dist_t$estimate[1]` A^o^; mean putative = `r dist_t$estimate[2]` A^o^; p = `r round(dist_t$p.value, 2)`).

To infer fitness cost in the putative resistance mutations, the stability of the protein was estimated as the change in Gibbs free energy ($\Delta\Delta$G). There was however, no significant difference between the known resistance mutations and the putative resistance mutations (mean known = `r fitness_t$estimate[1]`; mean putative = `r fitness_t$estimate[2]` p = `r round(fitness_t$p.value, 2)`). This result is perhaps surprising given the significant difference between known and putative mutations in terms of the percentages of samples with with compensatory mutations. 
...
In lieu of DST testing, we have provided evidence that our *katG* putative resistance mutations are causally resistance-conferring. 

Our mutations occur in multiple sublineages, indicating convergence independent of phylogeny, a pattern which is well established in the known *katG* resistance mutations.
	
No significant differences were found in the percentage of samples with (non-*katG*) known resistance mutation co-occurrence (our proxy for resistance level) between the known resistance mutations and our putative resistance mutations.
...
<!-- Drug resistance types and lineages were predicted *in silico* with TB-profiler (v4.2.0) [@Phelan2019] [@Coll2015]. TB-profiler software [@Phelan2019] was used to identify all known drug-resistance, compensatory and putative novel resistance mutations. -->
...
The open source software ChimeraX [@Pettersen2021] was used to model distances from the INH heme binding site and infer change in Gibbs free energy ($\Delta\Delta$G) *in silico*. 
...
cap <- sprintf("*Mycobacterium tuberculosis* samples (n=%s)", fmt(total_samps))
...
<!-- Figure 1 -->
```{r isoniazid-tree, echo=FALSE, out.width = "2000px", fig.cap=sprintf("Phylogenetic tree of isolates with resistance; Isoniazid resistance (n=%s), with potentially new katG mutations", inh_4_a_c_e_g)}
...
cap <- "Mutations in compensatory genes and frequencies in 32k samples. Underlined if known compensatory mutations"
...
cap <- "Percentages of co-occurring known resistance mutations and compensatory mutations among known and putative ('unknown') resistance mutations. Mutations in fewer than three samples are ommitted. Co-occurring known resistance mutations are in genes *fabG1*, *inhA*, *kasA*. The three categories of high medium and low resistance/fitness are based on tertiles of the respective percentages, except for the 'high' category of resistance level, which is based on the ~15% Ser315Thr resistance mutation co-occurrence. See **Figure S4**"
...
cap <- "Most common known resistance mutations for INH and RIF in TBprofiler. Mutations occurring less than five times are omitted."
...
```{r inh-ahpc-katg-links, echo=FALSE, warning = F, out.width = "2000px", fig.cap="Co-occurrences between *ahpC* compensatory mutation positions and *katG* PRM positions." }
...
```{r homoplasy-plot, echo=FALSE, warning = F, out.width = "2000px", fig.cap="Homoplasy among known and putative ('unknown') resistance mutations in *katG*, as indicated by number of sublineages in which mutations occur (y-axis). Ser315Thr is highlighted. Mutations in fewer than three samples are omitted."}
