...
Compensatory mutations are known to occur when mutations with high fitness costs occur (e.g. *katG* loss of function). To estimate the fitness impact of the `r inh_n_uniq_PRM_katg` putative resistance mutations we used the co-occurrence with a compensatory mutation as a proxy, comparing to *katG* Ser315Thr which has reported to confer a low fitness cost [@Pym2002] with only approximately ~3% of samples containing the mutation co-occurring with a compensatory mutation.  For simplicity of comparison with Ser315Thr and the 315 codon, we divided fitness cost into 'low', 'medium' and 'high' categories, based on tertiles of the percentage of samples with compensatory mutations (low:<=17%, medium:>17%<=40%, high:>40%<=100%). Indeed, mutations at the 315 codon position in general appear to have low fitness cost - **Figure S4** shows four out of the five known 315 resistance mutations appearing in our 'low' fitness cost category. 
...
Similarly, when comparing the percentage of samples, for any given resistance mutation, that have a compensatory mutation, there was a significant difference between the known (mean=19.6%) and putative (mean=51.5%) resistance mutation categories (p = 3×10-5). This difference remained significant even when excluding the 315 positions (p = 2×10-4).
